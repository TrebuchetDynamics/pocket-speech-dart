...
class OnnxModelRunner {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _trace('Using ONNX providers: $providers');
    ...
    try {
      ...
      try {
        ...
        if (inputNames.contains('input_ids')) {
          _trace('Using newer model format with input_ids');
          // Newer model format
          ...
          _trace('Using older model format with tokens');
          // Older model format
          // CRITICAL: Use Int64List for token IDs to match Python int64 type
      ...
      if (isInt8) {
        final List<int> outputList = rawList
        ...
        final List<double> outputList = rawList
