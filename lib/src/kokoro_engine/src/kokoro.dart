...
class Kokoro {
    ...
    try {
      ...
      for (final voiceName in voicesData.keys) {
        try {
              ...
              try {
                ...
                if (vector.isNotEmpty && vector.first is List) {
                  if (vector.length == 1) {
                    ...
                    // _trace('Info: Handling wrapped style vector for $voiceName.');
                  } else {
                    _trace(
          ...
          _trace('Error processing voice $voiceName: $e');
      ...
      _voices = voiceMap;
      _trace('Successfully loaded ${_voices.length} voices from voices.json');
    } catch (e) {
      throw Exception(
  ...
  Future<TtsResult> createTTS({
    ...
    _trace('Dart createTTS: Input text: "$text"');
    _trace(
    ...
    _trace('Dart createTTS: Resolved Voice ID: ${voiceObj.id}');
    ...
    _trace('Dart createTTS: Generated/Provided Phonemes: "$phonemes"');
    ...
    for (final batch in batches) {
      ...
      _trace('Using style vector for token length ${tokens.length}');
