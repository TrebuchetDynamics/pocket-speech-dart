...
List<_TextPart> _advancedSplit(String text) {
  ...
  final RegExp r = RegExp(
    r"([\wáéíóúüñÁÉÍÓÚÜÑ]+)|([^\wáéíóúüñÁÉÍÓÚÜÑ\s]+)|(\s+)",
  );
...
class Tokenizer {
  ...
  Future<void> _initialize() async {
    ...
    final String goldJson = await rootBundle.loadString(
      'packages/malsami/assets/us_gold.json',
    );
    lexicon.golds = lexicon.growDictionary(
      json.decode(goldJson) as Map<String, dynamic>,
    );

    final String silverJson = await rootBundle.loadString(
      'packages/malsami/assets/us_silver.json',
    );
    lexicon.silvers = lexicon.growDictionary(
      json.decode(silverJson) as Map<String, dynamic>,
    );
  ...
  Future<void> _loadVocabulary() async {
    try {
      ...
      _trace(
        'Loaded vocabulary with ${_vocab.length} entries',
  ...
  Future<void> _loadLexicon() async {
    try {
      ...
      _trace('Added custom entries to G2P lexicon', name: 'kokoro_tokenizer');
    } catch (e) {
      // Just log the error but don't fail - lexicon is optional
      _trace('Warning: Failed to load lexicon: $e', name: 'kokoro_tokenizer');
      _lexicon = {};
    }
  }
  ...
  Future<(String, List<MToken>)> phonemizeWithTokens(
    ...
    // _trace('DEBUG: Lowercased normalizedText for G2P: $normalizedText',
    //     name: 'kokoro_tokenizer');
    ...
    if (lang.startsWith('en')) {
      ...
      _trace(
        'Phonemes (phonemizeWithTokens): $phonemes',
        name: 'kokoro_tokenizer',
      );
      _trace(
    ...
    final (phonemes, tokens) = await _g2p.convert(normalizedText);
    _trace(
      'Phonemes (fallback in phonemizeWithTokens): $phonemes',
      name: 'kokoro_tokenizer',
    );
    return (phonemes, tokens);
  }
  ...
  Future<String> phonemize(String text, {String lang = 'en-us'}) async {
    ...
    for (final _TextPart part in parts) {
      switch (part.type) {
        case 'word':
          final wordToProcess = part.text;
          final phonemes = _isSpanish(lang)
              // ponytail: approximate Spanish G2P, replace with espeak/misaki when quality matters.
              ? _spanishPhonemes(wordToProcess)
              : await _englishPhonemes(wordToProcess);
          resultSegments.add(phonemes);
          break;
        case 'punctuation':
          resultSegments.add(part.text);
          _trace('Punctuation: "${part.text}"', name: 'kokoro_tokenizer');
          break;
        case 'whitespace':
          resultSegments.add(part.text);
          ...
          final String printableWhitespace = part.text
              ...
              .replaceAll('\r', '\\r');
          _trace(
            'Whitespace: "$printableWhitespace"',
            name: 'kokoro_tokenizer',
          );
          break;
      }
  ...
  Future<String> _englishPhonemes(String wordToProcess) async {
    final String? lexiconPhonemes = _lexicon[wordToProcess.toLowerCase()];
    if (lexiconPhonemes != null) return lexiconPhonemes;

    final (phonemes, /* List<MToken> tokens */ _) = await _g2p.convert(
      wordToProcess,
    );
    var phonemesFromG2P = phonemes;
    ...
    return phonemesFromG2P;
  }

  bool _isSpanish(String lang) {
  ...
  String _spanishPhonemes(String word) {
    final chars = _stripSpanishAccents(
      word.toLowerCase(),
    ).runes.map(String.fromCharCode).toList();
    final out = StringBuffer();
    var i = 0;
    while (i < chars.length) {
      final rest = chars.skip(i).join();
      if (rest.startsWith('ch')) {
        ...
        out.write('k');
        i += 2;
      } else if (rest.startsWith('gue') || rest.startsWith('gui')) {
        out.write('ɡ');
        i += 2;
      } else {
        out.write(_spanishLetterPhonemes[chars[i]] ?? chars[i]);
        i++;
      }
    }
    return out
        ...
        .where(_vocab.containsKey)
        .join();
  }

  String _stripSpanishAccents(String value) => value
      .replaceAll('á', 'a')
      .replaceAll('é', 'e')
      .replaceAll('í', 'i')
      .replaceAll('ó', 'o')
  ...
  static const _spanishLetterPhonemes = {
    'b': 'b',
    'c': 'k',
    'd': 'd',
    'f': 'f',
  ...
  void debugPunctuationProcessing(String text) async {
    ...
    _trace(
      'PUNCTUATION DEBUG: Original text: "$text"',
      name: 'kokoro_tokenizer',
    );

    // 2. Split by words and punctuation to test individually
    final RegExp wordOrPunctRegex = RegExp(
      r"([\w']+|[^\w\s]+)",
    ); // Match words or punctuation
    final matches = wordOrPunctRegex
        .allMatches(text)
        .map((m) => m.group(0)!)
        .toList();

    _trace(
      'PUNCTUATION DEBUG: Split into ${matches.length} parts:',
      name: 'kokoro_tokenizer',
    );
    for (int i = 0; i < matches.length; i++) {
      _trace(
        'PUNCTUATION DEBUG: Part $i: "${matches[i]}"',
        name: 'kokoro_tokenizer',
      );
    }
    ...
    for (int i = 0; i < matches.length; i++) {
      ...
      try {
        final (partPhonemes, partTokens) = await _g2p.convert(
          part.toLowerCase(),
        );
        _trace(
        ...
        _trace(
          'PUNCTUATION DEBUG: Error processing "$part": $e',
          name: 'kokoro_tokenizer',
        );
      }
    }
    ...
    for (int i = 0; i < matches.length - 1; i++) {
      ...
      try {
        final (comboPhonemes, comboTokens) = await _g2p.convert(
          combo.toLowerCase(),
        );
        _trace(
...
void _trace(Object? message, {String? name}) {}
